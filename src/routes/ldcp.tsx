...
function LDCPPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-full animate-fade-in space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">{data.length} symbols — Last Day Closing Prices from PSX</p>
        </div>
        <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:justify-end">
          <div className="relative flex-1 sm:flex-initial">
            ...
            <input
              ...
              className="h-10 w-full sm:w-64 rounded-lg border border-border bg-surface pl-9 pr-8 text-sm text-foreground placeholder:text-muted-foreground outline-none transition-all duration-200 focus:border-primary focus:ring-1 focus:ring-primary"
            />
            {search && (
              <button onClick={() => setSearch("")} className="btn-tight absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="flex md:hidden flex-wrap gap-1.5">
        {(["symbol", "price", "idxWt", "volume"] as const).map((k) => (
          <button
            key={k}
            onClick={() => handleSort(k)}
            className={`btn-tight rounded-full border px-3 py-1.5 text-[11px] font-medium transition-colors ${
              sortBy === k
                ? "border-primary/50 bg-primary/15 text-primary"
                : "border-border bg-surface text-muted-foreground"
            }`}
          >
            {k === "idxWt" ? "IDX %" : k.charAt(0).toUpperCase() + k.slice(1)}
      ...
      <div className="grid gap-2 md:hidden">
        {filtered.slice(0, visibleCount).map((d) => (
          <div key={d.symbol} className="rounded-xl border border-border bg-card p-3.5">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="text-base font-bold text-foreground">{d.symbol}</p>
                <p className="truncate text-xs text-muted-foreground">{d.company}</p>
              </div>
              <p className="font-mono text-base font-semibold text-foreground tabular-nums">{d.price.toFixed(2)}</p>
            </div>
            <div className="mt-3 grid grid-cols-3 gap-2 text-[11px]">
              <div>
                <p className="text-muted-foreground">IDX WT</p>
                <p className="font-mono text-foreground">{d.idxWt.toFixed(2)}%</p>
              </div>
              <div>
                <p className="text-muted-foreground">FF MCap</p>
                <p className="font-mono text-foreground">{(d.ffMcap / 1e9).toFixed(2)}B</p>
              </div>
              <div>
                <p className="text-muted-foreground">Volume</p>
                <p className="font-mono text-foreground">{d.volume.toLocaleString()}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="hidden md:block overflow-x-auto rounded-xl border border-border bg-card">
        <table className="w-full min-w-[760px] text-sm whitespace-nowrap">
          ...
          <tbody>
            {filtered.slice(0, visibleCount).map((d) => (
              <tr key={d.symbol} className="border-b border-border transition-all duration-200 hover:bg-surface-hover">
      ...
      {filtered.length > visibleCount && (
        <div className="text-center">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2.5 text-xs font-semibold text-foreground transition-all hover:bg-surface-hover hover:border-primary/40"
          >
